...
#!/usr/bin/env python3
import re, os, fnmatch, time, threading, logging, shutil
...
class TodoService:
        ...
        for fn in self._find_files():
            ...
            while i < len(lines):
                ...
                task = {
                    ...
                    'include': None, 'focus': None, 'knowledge': None,
                    '_start_stamp': None, '_know_tokens': 0,
                    '_prompt_tokens': 0, '_token_count': 0,
                }
                j = i + 1
        ...
        while True:
            for fn in self._find_files():
                ...
                if ignore and abs(mtime - ignore) < 0.001:
                    ...
                    logger.info(f"Detected change in {fn}, running /todo")
                    if self._svc:
                        try:
    ...
    def _start_task(task: dict, file_lines_map: dict):
        ...
        know = task['_know_tokens']; prompt = task['_prompt_tokens']; total = task['_token_count']
    ...
    def _complete_task(task: dict, file_lines_map: dict):
        ...
        stamp = datetime.utcnow().strftime('%Y-%m-%d %H:%M')
        start = task['_start_stamp']; know = task['_know_tokens']
        prompt = task['_prompt_tokens']; total = task['_token_count']
    ...
    def _resolve_path(self, raw_focus: str, include: dict) -> Optional[Path]:
        ...
        if not any(sep in frag for sep in (os.sep, "/", "\\")):
            logger.warning("Bare-filename focus '%s' is not supported", raw_focus)
            return None
        glob_frag = frag.replace("\\", "/")
        if any(c in glob_frag for c in ("*", "?", "[", "]")):
            pattern = re.compile(fnmatch.translate(glob_frag), re.IGNORECASE)
        else:
        ...
        roots_mapping = (include if isinstance(include, dict)
                         and include and all(isinstance(v, str) for v in include.values())
                         else {f"root_{i}": r for i, r in enumerate(self._roots)})
        for root in roots_mapping.values():
            base = Path(root)
            if not base.is_dir():
                continue
            for f in base.rglob("*"):
                if f.is_file() and pattern.search(str(f.resolve()).replace("\\", "/")):
                    return f.resolve()
        p = Path(frag); dir_frag = p.parent; filename = p.name
        if dir_frag.parts and filename:
            for root in roots_mapping.values():
                base = Path(root)
                try:
                    new_dir = (base / dir_frag).resolve()
    ...
    def _apply_focus(self, raw_focus: str, ai_out: str, svc, include: dict, target: Path):
        ...
        if bf:
            bdir = Path(bf); bdir.mkdir(parents=True, exist_ok=True)
            if target.exists():
                ts = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
                dest = bdir / f"{target.name}-{ts}"
    ...
    def _extract_focus(self, focus_spec) -> Optional[str]:
        if not focus_spec:
            return None
        raw = (focus_spec.get('file') or focus_spec.get('pattern')
               if isinstance(focus_spec, dict) else str(focus_spec))
        raw = raw.strip('"').strip("`")
    ...
    def _process_one(self, task: dict, svc, file_lines_map: dict):
        ...
        pt = self._get_token_count(prompt); total = kt + pt
        task['_know_tokens'] = kt; task['_prompt_tokens'] = pt; task['_token_count'] = total
