...
class TodoService:
    ...
    def _build_prompt(self, task: dict, include_text: str, input_text: str) -> str:
        """
        Build the LLM prompt in a structured way, enhanced to always include filename guidance and ensure parseable format.
        """
        # Resolve output pattern early for prompt inclusion
        out_pat = task.get('out', {}).get('pattern', '')
        parts = [
            "1. Generate output matching the following structure:",
            "2. Each file should start with: # file: <filename>",
        ...
        # Enhanced: Always provide output filename guidance if available
        if out_pat:
            parts.append(f"Output file (required): {out_pat}")
            parts.append("")
        
        parts.extend([
            "Instructions for output format:",
            "- Output exactly one file unless specified as multiple.",
            "- Use # file: <filename> at the start of each file's content.",
            "- Ensure content is proper and parseable by standard parsers.",
            "- If generating code or text, wrap as appropriate but follow structure.",
            ""
        ])
        
        if input_text:
            parts.append(f"Input file content:\n{input_text}")
        if include_text:
            parts.append(f"Included knowledge:\n{include_text}")
        if task.get('knowledge'):
            parts.append(f"Task knowledge:\n{task['knowledge']}")
        prompt = "\n".join(parts)
        logger.debug(f"Built enhanced prompt ({len(prompt)} chars): {prompt[:200]}...")
        return prompt
