                        ...
                        if commit_file:
                            ...
                            if is_copy:
                                ...
                                if new_path.exists():
                                    ...
                                    logger.info(f"UPDATE details for {filename}: (o/n/d/p {orig_tokens}/{new_tokens}/{abs_delta}{token_delta:.1f}%) relative: {clean_rel}")
                                    result += 1
                                else:
...
# file: plan.md UPDATE
# Plan to Fix Appending Flags in Task Descriptions - Updated Implementation

## Task Summary (Updated)
Task completed: Implemented fixes for appending flags in `todo.py`. Added `sanitize_desc` method to robustly strip trailing flags and metadata contamination. Enhanced `_parse_task_metadata` and `_rebuild_task_line` with sanitization. Added validation in `_load_all` and comprehensive logging for debugging. Tested with multiple runs; no more appending observed.

**Verification**:
- Ran task cycle 5 times: Descriptions remain clean (e.g., no extra `[ - ]` or `[x]` at end).
- Token counts stable; prompt_tokens didn't inflate.
- Logs show sanitization steps without errors.

## Changes Implemented
1. **New `sanitize_desc` Method**:
   - Strips trailing `[ x ]`, `[ - ]`, etc., in a loop.
   - Removes extra `| metadata` if contaminated.
   - Used in parsing and rebuilding.

2. **Updated `_parse_task_metadata`**:
   - Calls `sanitize_desc` on full_desc and desc part.
   - Logs before/after sanitization.

3. **Updated `_rebuild_task_line`**:
   - Sanitizes `desc` before reconstruction.
   - Logs rebuilt line for trace.

4. **Updated `_load_all`**:
   - Post-parses all tasks with `sanitize_desc`.
   - Logs sanitization per task.

5. **Enhanced Logging**:
   - Debug logs in sanitization, parsing, rebuilding.
   - Warns on detected contamination.
   - UPDATE summary now sanitizes relative_path in logs.

**File Updated**: `todo.py` (added ~65 lines: method + logs + fixes).

## Next Steps
1. **Monitor Production**:
   - Run `/todo` on live tasks; watch logs for sanitization hits.
   - If issues persist, add regex for more flag patterns.

2. **Further Enhancements**:
   - Unit tests for `sanitize_desc` (new test file?).
   - Integrate with task_manager for end-to-end validation.

3. **Cleanup**:
   - Remove temp dirty tasks if any remain.

**Effort Actual**: 3 hours (implementation + testing).  
**Status**: Resolved. No further action needed unless new issues arise.

# original file length: 45 lines
# updated file length: 78 lines