...
class TodoService:
    ...
    def _start_task(task: dict, file_lines_map: dict, cur_model: str):
        ...
        summary = TodoService._format_summary(indent, stamp, None, know, prompt, incount, include, cur_model)
        idx = ln + 1
    ...
    def _complete_task(task: dict, file_lines_map: dict, cur_model: str):
        ...
        know, prompt, incount, include = (task.get(k, 0) for k in
                                          ('_knowledge_tokens', '_prompt_tokens', '_in_count', '_include_tokens'))
        summary = TodoService._format_summary(indent, stamp, stamp, know, prompt, incount, include, cur_model)
        idx = ln + 1
        if idx < len(file_lines_map[fn]) and file_lines_map[fn][idx].lstrip().startswith('- started:'):
            file_lines_map[fn][idx] = summary
        else:
    ...
    def _process_one(self, task: dict, svc, file_lines_map: dict):
        ...
        # Enhanced prompt for better ParseService compatibility
        prompt_instructions = """
When making changes, ALWAYS respond with COMPLETE FILE CONTENTS using this format:

# file: relative/path/to/file.ext
Full file content here...

Include ALL lines of each file you modify, even unchanged ones. If multiple files, list each separately.
"""
        parts = []
        if incontent:
