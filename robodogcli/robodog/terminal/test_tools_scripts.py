# file: terminal/test_tools_scripts.py
"""
Tests for streaming bash + run_script in terminal/tools.py.
...
powershell, run_script timeout, and a regression run of terminal/selftest.py.

Run:  python terminal/test_tools_scripts.py    (from robodogcli/robodog/)
"""
...
def main() -> int:
    ...
    proc = subprocess.run(
        [sys.executable, "terminal/selftest.py"],
        cwd=str(ROOT), capture_output=True, text=True, timeout=300,
    )
    check(proc.returncode == 0,
          f"terminal/selftest.py exits 0 (got {proc.returncode})")
