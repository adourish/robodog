...
research), closing most of the gap to Qwen Code — robodog's detection
is still tag-based rather than Qwen's JSON-depth streaming parser (which
catches truncation even when the provider omits `finish_reason: "length"`
entirely), so Qwen Code's remains the more complete of the two. robodog
is otherwise on par with Cline and Aider, and ahead of Roo Code (which
detected malformed truncated JSON but explicitly forces the call through
anyway), goose, and Continue.dev (neither handles it at all). Session
