...
it. On truncated-response resilience robodog now also auto-retries with
an escalated `max_tokens` ceiling (0.3.75, prompted directly by this
research). Robodog's own detection was already independent of
`finish_reason` (an unclosed tag is a structural signal, checked
regardless of what the provider reports) — different tools, different
formats to detect truncation in, same underlying idea. Qwen Code's real
edge is architectural, not a design flaw on robodog's side: it runs over
a *streamed* response, so its JSON-depth tracker can flag an incomplete
tool call mid-generation, before the response even finishes. Robodog's
non-streaming backend path has no "mid-stream" to detect anything in —
it only ever sees a complete or cut-off response, never a partial one in
flight. robodog is otherwise on par with Cline and Aider, and ahead of
Roo Code (which detected malformed truncated JSON but explicitly forces
the call through anyway), goose, and Continue.dev (neither handles it at
all). Session
